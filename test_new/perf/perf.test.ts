...
async function installPerfHelpers(page: Page) {
  await page.evaluate(() => {
    ...
    function runPerf(engineName: 'native' | 'nw'): PerfResult[] {
      ...
      return [
        bench('match div miss', () => engine.match('div', targets.button), 5_000_000),
        bench('match div hit', () => engine.match('div', targets.div), 5_000_000),
        bench('match button hit', () => engine.match('button', targets.button), 5_000_000),
        bench('match body hit', () => engine.match('body', targets.body), 5_000_000),
      ];
...
test('blob fixture comparison', async ({ browser }) => {
  ...
  printComparison(all, 'nw-current', probeKeys);
});

function printComparison(all: Record<EngineName, PerfResult[]>, currentName: EngineName, probeKeys: string[]) {
  ...
  const rows = current.map((cur) => {
    ...
    for (const [name, results] of Object.entries(all)) {
      ...
      row[name] = base ? ratio(cur.ms, base.ms) : 'missing';
    }
