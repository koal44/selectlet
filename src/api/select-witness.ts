...
import type { ComplexPart, ComplexSelector, CompoundSelector, SelectorList } from '../parser/parser';
...
import { describeCombinator, describeContext, describeElements, describeFilter, describeLookup } from '../utils/debug';
...
function buildWitnessProgram(complex: ComplexSelector, snap: Snapshot): WitnessProgram {
  const { parts } = complex;
...
function getAdvanceMove(program: WitnessProgram, index: number, snap: Snapshot): AdvanceMove | null {
...
function getBridgeMove(program: WitnessProgram, index: number, snap: Snapshot): BridgeMove | null {
  const step = program.steps[index];
...
function canAdvance(state: WitnessState): boolean {
...
function runAdvanceMove(state: WitnessState, move: AdvanceMove, canRoot: boolean, rc: RuntimeCache | null): void {
...
function runBridgeMove(state: WitnessState, move: BridgeMove, canRoot: boolean, rc: RuntimeCache | null): void {
...
function describeWitnessProgram(program: WitnessProgram): DebugWitnessProgram {
...
function resetWitnessDebug(program: WitnessProgram): void {
